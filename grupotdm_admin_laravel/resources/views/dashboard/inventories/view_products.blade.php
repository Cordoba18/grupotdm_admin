...
@section('content')
    ...
    <div id="carouselExampleRide" class="carousel slide" data-bs-ride="true">
        <div class="carousel-inner">
            ...
            @foreach ($images_product as $ip)
                ...
                @endphp
                <div
                    class="@if ($acumulador == 1) {{ 'carousel-item active' }}@else{{ 'carousel-item' }} @endif">
    ...
    <br>
    <form id="miFormulario" action="{{ route('dashboard.inventories.view_product.save_changes_view_product') }}"
        method="POST" enctype="multipart/form-data">
        @csrf
        <input readonly hidden type="text" required name="id_product" class="form-control" id="exampleFormControlInput1"
            placeholder="" value="{{ $product->id }}">
        <div class="mb-3">
            <label for="exampleFormControlInput1" class="form-label">ID</label>
            <input disabled readonly type="text" required name="id" class="form-control"
                id="exampleFormControlInput1" placeholder="" value="{{ $product->id }}">
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="text" required name="brand" class="form-control" id="exampleFormControlInput1" placeholder=""
                value="{{ $product->brand }}">
        </div>
        <div class="mb-3">
            <label for="exampleFormControlTextarea1" class="form-label">Serial</label>
            @if ($product->id_type_component == 2)
                <input type="text" disabled name="" class="form-control" id="exampleFormControlInput1"
                    placeholder="" value="{{ $product->serie }}">
            @else
                <div class="content_code_references" id="content_code_references"
                    style="width: 100%; height: 300px; display: flex; flex-wrap: wrap; align-items: center;">
                    <div class="content_code_refences_header"
                        ...
                        <div class="content_logo" style="width: 300px; height: 100px;">
                            <img style="width: 100%; height: 100%; object-fit: contain;"
                                src="{{ asset('storage/icons/logo.png') }}" alt="">
                        </div>
                        <div class="content_references_img" style="display: flex; flex-wrap: wrap; justify-content: center">
                            ...
                            <div class="content_text_references"
                                style="text-align: center; width: 100%; flex-direction: column; display: flex;"><b
                                    style="font-size: 40px;">{{ $product->serie }}</b><b
                                    style="width: 100%;font-size: 25px;">{{ $product->name }} ID {{ $product->id }} </b>
                            </div>
                        </div>
                    </div>
                ...
                <button type="button" href="" onclick="imprimirDiv()" class="btn btn-dark"
                    style="width: 100%">IMPRIMIR REFERENCIA</button>
            @endif
        </div>

        <div class="mb-3">
            <label for="exampleFormControlTextarea1" class="form-label">Accesorios</label>
            <textarea required class="form-control" id="exampleFormControlTextarea1" rows="3" name="accessories"
                placeholder="Ingrese observaciones" maxlength="500">{{ $product->accessories }}</textarea>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" required disabled readonly name="" class="form-control"
                id="exampleFormControlInput1" placeholder="" value="{{ $state }}">
        </div>
        <div class="mb-3">
            ...
            <a href="{{ route('dashboard.users.view_user', $user_create_product->id) }}">
                <input type="text" required
                    style="width: 100%; padding: 10px; background-color: white; border: 1px solid black; font-weight: bold;"
                    name="" class="form-control" id="exampleFormControlInput1" placeholder=""
                    value="{{ $user_create_product->name }}">
            </a>
        </div>
        @if ($validate_user_sistemas)
            ...
            <div class="content_buttons" style="display: flex; flex-wrap: wrap; width: 100%;">
                <button style="margin: 5px; width: 100%;" class="btn btn-success">Guardar Cambios <i
                        class="bi bi-box-arrow-down"></i></button>
                <a style="margin: 5px; width: 100%;"
                    href="{{ route('dashboard.inventories.view_product.images_product', $product->id) }}"
                    class="btn btn-primary"> IMAGENES SECUNDARIAS <i class="bi bi-image"></i></a>
                <br>
    </form>
    <form style="width: 100%;" action="{{ route('dashboard.inventories.delete') }}" method="post"
        @if ($product->id_state == 1) onsubmit="return confirmarEnvio() @endif">
        @csrf
        <input type="text" hidden value="{{ $product->id }}" name="id_product">
        @if ($product->id_state == 1)
            <button style="margin: 5px; width: 100%; text-align: center;" class="btn btn-danger"><i
    ...
    <form action="{{ route('dashboard.inventories.view_product.characteristic_product.save') }}" method="post">

        @csrf
        <input type="number" hidden value="{{ $product->id }}" name="id_product">

        <div class="mb-3">
            <label for="formFile" class="form-label">AGREGAR CARACTERISTICA</label>
            <input  type="text" required name="type" class="form-control"
                id="exampleFormControlInput1" placeholder="Ingrese el tipo de caracteristica">
                <input type="text" required name="detail" class="form-control"
                id="exampleFormControlInput1" placeholder="Ingrese el detalle">
          </div>

          <button class="btn btn-primary" style="margin-bottom: 5px; width: 100%;">GUARDAR CARACTERISTICA</button>
    </form>
    ...
    <table id="miTabla" class="table table-bordered table-striped dataTable">
        ...
        @foreach ($characteristic_products as $c)
        <tr>
            <td>{{ $c->type }}</td>
            <td>{{ $c->detail }}
                @if($validate_user_sistemas)
                <td><form action="{{ route('dashboard.inventories.view_product.characteristic_product.delete') }}" method="post">
                    @csrf
                    <input type="number" value="{{ $c->id }}" name="id_characteristic_product" hidden>
                    <input type="number" value="{{ $product->id }}" name="id_product" hidden>
                @csrf
        ...
        <tbody>
            @foreach ($reports_product as $r)
                <tr>
                    <td>{{ $r->id }}</td>
                    <td><b>{{ $r->report }}</b></td>
                    <td><a href="{{ route('dashboard.certificates.view_certificate', $r->id_certificate) }}"
    ...
    <script>
        function imprimirDiv() {
            ...
            html2canvas(contenidoDiv).then(function(canvas) {
                ...
                ventanaImpresion.document.write('<img src="' + imgData +
