...
@section('content')
<form action="{{ route('dashboard.tickets.save_changes_ticket') }}" method="POST" enctype="multipart/form-data">
    @csrf
    <input type="number" name="id_ticket" hidden value="{{ $ticket->id }}">
<div class="mb-3">
    <label for="exampleFormControlInput1" class="form-label">Nombre</label>
    <input type="text" name="name" class="form-control" id="exampleFormControlInput1" placeholder="Ingrese nombre del ticket" required value="{{ $ticket->name }}">
  </div>
  <div class="mb-3">
    <label for="exampleFormControlTextarea1" class="form-label">Descripciòn</label>
    <textarea  required class="form-control" id="exampleFormControlTextarea1" rows="3" name="description" placeholder="Ingrese una descripcion" maxlength="500" >{{ $ticket->description }}</textarea>
  </div>
...
@endphp
  ...
  <div class="mb-3 row">
    ...
    <div class="col-sm-10">
      <input disabled type="text" id="input_date_start" name="" readonly class="form-control-plaintext" value="{{ $ticket->name_destination }}">
    </div>
  @endif
  <div class="mb-3 row">
    <label for="staticEmail" class="col-sm-2 col-form-label">Previa fecha de inicialización</label>
    <div class="col-sm-10">
      <input disabled type="text" id="input_date_start" name="" readonly class="form-control-plaintext" value="{{ $ticket->date_start }}">
    </div>
</div>
  <div class="mb-3 row">
    <label for="staticEmail" class="col-sm-2 col-form-label">Previa fecha de finalización</label>
    <div class="col-sm-10">
      <input disabled type="text" name="" id="" readonly class="form-control-plaintext" value="{{ $ticket->date_finally }}">
    </div>
</div>
